...
## Session — v2.3 (US-16 recurring transactions), overnight autonomous run continued (2026-07-15)

**Trigger/context change mid-session:** dispatched `architect-pondo` for the v2.3 design; its reference-gathering subagent completed successfully, but the main run then hit a genuine Ollama Cloud `429 session usage limit` (account-level, not per-agent) before writing the actual design doc. Rather than block the sprint retrying against a live external quota, DARKLING wrote the design directly (`docs/v2.3-recurring-transactions/01-recurring-design.md`) using the same reference material the subagent had already pulled, then built the entire v2.3 implementation directly as well rather than risk further blocked dispatches. Gino later confirmed the Ollama limit had reset; TWC/OpenClaw dispatch remains available again for v2.4/v2.5 if preferred.

### Design — recurrences table, cron sweep, confirm-mode, all written directly

`recurrences` table (repeat/installment/subscription modes, weekly/monthly cycle only — no `custom`, since a recurrence is inherently periodic), `entries.recurrence_id` (nullable FK, same shape as `transfer_group_id`). Due-date math reuses the same clamping primitives as `budget-cycle.js` rather than re-deriving them (extracted into a new shared `server/lib/date-utils.js`). Confirm-mode (`auto_post = false`) tracked via `pending_confirmation` + `pending_due_date` columns — simpler and unambiguous compared to the original design draft's plan to infer pending state by comparing entries-table gaps, which turned out to be underspecified once implementation started and was revised in the doc before being built.

**Design-time catch:** the original design draft had the recurrence-confirm action calling the generic `POST /api/entries` with a client-supplied `recurrence_id` — caught during implementation as inconsistent with the codebase's own established precedent (`transfer_group_id` is never client-settable through the generic entries API, only through dedicated RPC functions). Fixed before writing any route code: confirmation now goes through a dedicated `POST /api/recurrences/:id/confirm` endpoint, and `createEntry()` was extended with an internal-only second parameter so `recurrence_id` can never be forged via a POST body.

### Two defects found and fixed during implementation (beyond the design-time catch above)

1. **Vercel Cron auth was broken for both the new and the pre-existing purge cron.** Verified against Vercel's own docs: Cron Jobs invoke via HTTP GET only, and can only send Vercel's own `Authorization: Bearer $CRON_SECRET` header — never a custom header. The v2.1 `/api/recycle-bin/purge` endpoint was POST-only and X-API-Key-only, meaning its own `vercel.json` cron entry could never have actually authenticated successfully in production, undetected all session because production deploys have been held throughout. Fixed both `/purge` and the new `/api/recurrences/process` to accept GET+POST and either auth scheme. **New deployment prerequisite: a `CRON_SECRET` env var must be set on the Vercel project** (in addition to the existing `PURGE_API_KEY`) before either cron can succeed in production.
2. **Deeper cycle-math bug in the already-committed `budget-cycle.js` (`e3346c4`).** Both the previous-month fallback and the cycle-end calculation called `addMonths()` on a Date whose day-of-month could be 29/30/31 — which silently overflows past the intended month when that day doesn't exist there (e.g. Mar 31 shifted -1 month raw-Date-wise lands back on Mar 3, not Feb), and `clampToMonth` couldn't correct it since it re-derives year/month from the already-overflowed date. This is the same overflow bug class the v2.2 session believed it had already fixed — the difference is the v2.2 fix was verified with a single hand-traced example (April→March) that happened not to trigger this specific case. Found this time while implementing `recurrence-cycle.js`'s equivalent logic and re-examining `budget-cycle.js` as the reuse source. Fixed by adding `shiftMonth()` (pure integer year/month arithmetic, never constructs an intermediate Date, so it can't inherit JS's day-overflow behavior) to `date-utils.js`, and rewriting both `budget-cycle.js` call sites plus `recurrence-cycle.js`'s two month-shifting functions to use it. **Verified exhaustively this time, not by spot-check** — a throwaway Node script (kept in the session scratchpad, not committed) traced every anchor day 28 through 31 across all 12 months of the year, plus a 24-consecutive-month forward walk and a 12-occurrence installment end-date calculation. All passed. This changes the confidence level on the v2.2 budgets cycle math from "spot-checked" to "exhaustively verified" — the underlying bug predates this session's v2.3 work but was live in production-bound code since `e3346c4`.

### Verification

All 8 touched/new backend files passed `node --check`. `npm run build` reproduced locally twice (before and after the cycle-math fix), both clean, 1916 modules. Frontend (`Recurrences.jsx` CRUD management page modeled on `Budgets.jsx`, `RecurrenceConfirmCard.jsx` dashboard card positioned above `BudgetCard` per the design's "action items outrank passive monitoring" rationale, `api.js` exports, nav entry) reviewed directly against the design and codebase conventions rather than dispatched, given the Ollama quota uncertainty.

### Committed, not pushed

One local commit (`d3d8320`) covering the full v2.3 design + backend + frontend + the retroactive `budget-cycle.js` correction. Same standing hold as every prior session: migration `005_recurring_transactions.sql` is written but not applied; no push, no Vercel deploy, no production DDL.
